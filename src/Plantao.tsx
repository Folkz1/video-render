...
// Crossfade entre segmentos (fix jul14 — Diego: "a transição entre a voz do ElevenLabs e meus
// clipes não está boa"). Segmentos vizinhos se sobrepõem por OVERLAP frames; cada cena faz
// fade de OPACIDADE e de VOLUME nos primeiros/últimos OVERLAP frames → dissolve visual + cross-
// fade de áudio (a voz clonada some enquanto o áudio nativo do clipe entra), sem corte seco.
const OVERLAP = 6; // ~0.2s @30fps: imperceptível na fala, mata o "estalo" do corte

const fadeEnvelope = (frame: number, seqDur: number, fade: number): number => {
  // fade nunca passa de metade do segmento → range sempre monotônico (segmentos curtos safe).
  const f = Math.max(1, Math.min(fade, Math.floor(seqDur / 2)));
  return interpolate(frame, [0, f, seqDur - f, seqDur], [0, 1, 1, 0], {
...
export type PlantaoProps = {
  ...
  bgMusicUrl?: string; // cama de trilha bem baixa no fundo (Kit de Editor)
  bgMusicVol?: number; // volume da cama (default 0.04 — só um respiro no fundo, não compete com a fala)
  segments: Segment[];
};
...
// ─────────────────────────── Segmento VO ───────────────────────────
const VoScene: React.FC<{ seg: VoSegment; paleta: string[]; portrait: boolean; width: number; height: number; seqDur: number; fade: number }> = ({ seg, paleta, portrait, width, height, seqDur, fade }) => {
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: bg }}>
      ...
      {seg.audioUrl ? (
        <Audio src={resolveSrc(seg.audioUrl)} volume={(f) => fadeEnvelope(f, seqDur, fade)} />
      ) : null}
    </AbsoluteFill>
  );
};
...
const ClipScene: React.FC<{ seg: ClipSegment; paleta: string[]; portrait: boolean; seqDur: number; fade: number }> = ({ seg, paleta, portrait, seqDur, fade }) => {
  const frame = useCurrentFrame();
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: '#05060a' }}>
      <OffthreadVideo
        ...
        volume={(vf) => fadeEnvelope(vf, seqDur, fade)}
        style={{
          width: '100%',
          height: '100%',
          objectFit: 'cover',
          ...
          // GRADE cinematográfico do apresentador (Diego jul14: "usa alguma máscara/filtro pra quando
          // eu apareço"): leve contraste+saturação+brilho pra dar acabamento consistente.
          filter: 'contrast(1.08) saturate(1.10) brightness(1.02)',
        }}
      />
      {/* MÁSCARA/VINHETA: escurece as bordas e puxa o olho pro rosto (look de estúdio). Radial
          transparente no centro → sombra nas quinas + tônus frio da paleta bem sutil embaixo. */}
      <AbsoluteFill
        style={{
          background: `radial-gradient(ellipse ${portrait ? '80% 60%' : '62% 78%'} at 50% 42%, transparent 45%, rgba(3,5,9,0.42) 100%)`,
          pointerEvents: 'none',
        }}
      />
      <AbsoluteFill style={{ background: `linear-gradient(180deg, transparent 62%, ${accent}0D 100%)`, pointerEvents: 'none' }} />
...
// ─────────────────────────── Crossfade de opacidade entre segmentos ───────────────────────────
const FadeWrapper: React.FC<{ seqDur: number; fade: number; children: React.ReactNode }> = ({ seqDur, fade, children }) => {
  const frame = useCurrentFrame();
  const op = fadeEnvelope(frame, seqDur, fade);
  return <AbsoluteFill style={{ opacity: op }}>{children}</AbsoluteFill>;
};

// ─────────────────────────── Cama de trilha (bem baixa, o vídeo inteiro) ───────────────────────────
// SEM loop de propósito (Diego jul14: 'em loop ficou estranho') — o montador manda uma faixa longa
// (>= duração do vídeo), então toca UMA vez, do início ao fim, sem emenda/repetição.
const BgMusic: React.FC<{ src: string; vol: number }> = ({ src, vol }) => {
  const { durationInFrames } = useVideoConfig();
  // fade-in ~0.7s no começo e fade-out ~1s no fim pra não entrar/sair no talo.
  return (
    <Audio
      src={resolveSrc(src)}
      volume={(f) =>
        interpolate(f, [0, 20, Math.max(21, durationInFrames - 30), durationInFrames], [0, vol, vol, 0], {
          extrapolateLeft: 'clamp',
          extrapolateRight: 'clamp',
        })
      }
  ...
  return (
    <AbsoluteFill style={{ backgroundColor: paleta?.[0] ?? DEFAULT_PALETA[0] }}>
      {segments.map((seg, i) => {
        ...
        acc += durFrames; // acumulador avança pela dur NATURAL (não conta o overlap)
        // segmentos não-finais estendem OVERLAP frames pra dentro do próximo → crossfade real
        // (enquanto este some, o próximo já está entrando por cima). fade uniforme in/out.
        const seqDur = durFrames + (i < lastIdx ? OVERLAP : 0);
        return (
          <Sequence key={`seg${i}`} from={from} durationInFrames={seqDur}>
            <FadeWrapper seqDur={seqDur} fade={OVERLAP}>
              {seg.type === 'clip' ? (
                <ClipScene seg={seg as ClipSegment} paleta={paleta} portrait={portrait} seqDur={seqDur} fade={OVERLAP} />
              ) : (
                <VoScene seg={seg as VoSegment} paleta={paleta} portrait={portrait} width={width} height={height} seqDur={seqDur} fade={OVERLAP} />
              )}
            </FadeWrapper>
          </Sequence>
        );
      ...
      {/* cama de trilha bem baixa no fundo (o vídeo inteiro) */}
      {props.bgMusicUrl ? <BgMusic src={props.bgMusicUrl} vol={props.bgMusicVol ?? 0.04} /> : null}
    </AbsoluteFill>
  );
};
